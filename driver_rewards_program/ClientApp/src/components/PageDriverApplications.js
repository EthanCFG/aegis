...
function PageDriverApplications(props) {
  ...
  useEffect(() => {
    async function fetchApps() {
      const app_response = await axios.post(
        "http://localhost:3001/sponsor_application_list",
        {
  ...
  useEffect(() => {
    async function getDriver() {
      ...
      const add_driver_response = await axios.post(
        "http://localhost:3001/accept_driver_application",
        {
      ...
      const remove_application_response = await axios.post(
        "http://localhost:3001/remove_driver_application",
        {
          driver_id: selectedDriver,
          org_name: localStorage.getItem("sponsorname"),
        }
      ...
      //send message that driver was accepted
      const message_response = await axios.post(
        "http://localhost:3001/send_message",
        {
          ...
          messageText:
            "Your application to " +
            localStorage.getItem("sponsorname") +
            " has been accepted!",
  ...
  useEffect(() => {
    async function rejectDriver() {
      ...
      //send message that driver was rejected
      const message_response = await axios.post(
        "http://localhost:3001/send_message",
        {
          driverID: selectedDriver,
        ...
        <div class="columns is-centered">
          <div class="columns is-half">
            <div class="notification is-white py-3 my-6">
              <h1
                class="signup-header py-2 is-size-2 has-text-weight-semibold"
                style={{
                  marginBottom: 30,
                }}
              >
              ...
              {driverapplications.map((application) => {
                ...
                return (
                  <div class="box" key={ApplicationID}>
                    ...
                    <Button onClick={() => setSelectedDriver(DriverID)}>
                      Accept
                    </Button>{" "}
                    <Button onClick={() => setRejectedDriver(DriverID)}>
                      Reject
                    </Button>
                  </div>
                );
