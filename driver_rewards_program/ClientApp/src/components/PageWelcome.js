...
function PageWelcome({ setToken }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const sponsor_response = await axios.post(
      "http://localhost:3001/login_sponsor",
      {
        email: enteredEmail,
        password: enteredPassword,
      }
    ...
    const admin_response = await axios.post(
      "http://localhost:3001/login_admin",
      {
        email: enteredEmail,
        password: enteredPassword,
      }
    ...
    if (driver_response.data[0] != null) {
      ...
      if (org1_response.data[0] != null) {
        ...
        localStorage.setItem(
          "orgactive_ID",
          org1_response.data[0].Organization_ID
        );
      } else {
      ...
      for (let i = 0; i < org_list_response.data.length; i++) {
        if (
          org_list_response.data[i].Organization_ID ==
          sponsor_response.data[0].Organization_ID
        ) {
          localStorage.setItem(
            "sponsorname",
            org_list_response.data[i].Organization_Name
          );
        }
      }
      ...
      localStorage.setItem(
        "sponsoremail",
        sponsor_response.data[0].Sponsor_Email
      );
      localStorage.setItem(
        "sponsorfirst",
        sponsor_response.data[0].Sponsor_First_Name
      );
      localStorage.setItem(
        "sponsorlast",
        sponsor_response.data[0].Sponsor_Last_Name
      );
      setToken(token);
      ...
      const all_drivers_response = await axios.post(
        "http://localhost:3001/get_all_drivers"
      );
        ...
        <div class="columns is-half">
          <div class="column">
            <div class="notification is-white py-3 my-6">
              <img
                id="welcome-logo"
                src="https://1.bp.blogspot.com/-5uFQq0w64Hg/YVC6wgzpm0I/AAAAAAAAAAU/5q9okcQ4FJgFogvA0oQsauC85fo7mqDBwCLcBGAsYHQ/s841/Aegis%2BLogo%2BTemp%2BCopy.png"
              ></img>
              <script
                defer
              ...
              <div class="columns is-centered">
                <form onSubmit={handleSubmit}>
                  <button type="submit" class="button is-info my-3">
                    Sign In
                  </button>
                </form>
              </div>
